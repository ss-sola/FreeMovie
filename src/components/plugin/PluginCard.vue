  ...
  <ion-card class="card">
    <ion-grid>
      <ion-row>
        ...
        <ion-col size="auto"
          ><ion-toggle :checked="plugin.enable" @ionChange="enableChange(plugin, $event, 'enable')"
            >启用</ion-toggle
          ></ion-col
        >
      </ion-row>
      ...
      <ion-row>
        <ion-col size="5" @click="doUpdate(plugin)"
          ><i class="iconfont icon-jianchagengxin"></i>更新</ion-col
        >
        <ion-col size="5" @click="doRemove(plugin)"
          ><i class="iconfont icon-changyonggoupiaorenshanchu"></i>删除</ion-col
        >
      </ion-row>
    </ion-grid>
  </ion-card>
